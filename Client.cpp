...
addition::addition(string num, string plc, string dat, string sur, string nam,
    string pat, int bir, string cit, string str, string hos, string flt) {
...
drevo* add(drevo*& Root, addition ad) {
    drevo* cur = Root;
    if (cur == NULL) {
        drevo* p = new drevo;
        p->right = NULL;
        p->left = NULL;
        p->client.number = ad.num;
        p->client.place = ad.plc;
        p->client.date = ad.dat;
        //p->client.surname = ad.sur;
        ad.nam = ad.sur +" "+ ad.nam +" "+ ad.pat;
        p->client.name = ad.nam;
        //p->client.patronymic = ad.pat;
        p->client.birthday_year = ad.bir;
        p->client.adress_city = ad.cit;
        p->client.adress_street = ad.str;
        p->client.adress_house = ad.hos;
        p->client.adress_flat = ad.flt;
        p->height = 1;
        return p;
    }
    if (cur->client.number == ad.num) {
        cout << "Пользователь уже зарегистрирован, добавление снова не возможно" << endl;
        return cur;
    }
    if (ad.num < cur->client.number) {
        cur->left = add(cur->left, ad);
    }
    else {
        cur->right = add(cur->right, ad);
    }
    return balance(cur);
}
...
void treeprint(drevo*& tree) {
    if (tree != NULL) {
        ...
        cout << "Адресс регистрации: " << tree->client.adress_city <<" " << tree->client.adress_street << " " << tree->client.adress_house << " " << tree->client.adress_flat << endl;
        k++;
...
void ClientChoise()
{
    ...
    string pass_adress_flat;
        ...
        if (enter == 1) {
            ...
            addition ad(pass_num, pass_place, pass_date, pass_surname, pass_name, pass_patronymic,
                pass_year, pass_adress_city, pass_adress_street, pass_adress_house, pass_adress_flat);
            obj = add(obj, ad);
